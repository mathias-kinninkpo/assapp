  ...
  const handleSendDocument = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        // Succès - Redirection automatique vers le suivi
        const documentTypeName = getDocumentTypeName(selectedDocType);
        
        Alert.alert(
          '✅ Document envoyé !',
          `Votre ${documentTypeName} a été envoyé avec succès.\n\nVous allez être redirigé vers le suivi pour voir son état.`,
          [
            {
              text: 'Voir le suivi maintenant',
              onPress: () => {
                // Redirection vers la page de suivi avec les paramètres
                router.replace({
                  pathname: '/tracking',
                  params: {
                    fromUpload: 'true',
                    newDocumentId: result.document?.id || 'latest', // ID du document si disponible
        ...
        // Auto-redirection après 3 secondes si pas d'action
        setTimeout(() => {
          router.replace({
            pathname: '/tracking',
            params: {
              fromUpload: 'true',
              newDocumentId: result.document?.id || 'latest',
              documentType: selectedDocType,
              documentName: documentTypeName
            }
          });
      ...
      Alert.alert(
        'Erreur', 
        'Erreur inattendue lors de l\'envoi du document',
        [
          { text: 'Réessayer', onPress: () => {} },
          { 
            text: 'Voir mes documents', 
